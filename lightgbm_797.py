...
df.to_hdf('../../data/hc/df_1516.h5','lightgbm_data')'''
...
def cv_scores(df, num_folds, params, stratified = False, verbose = -1, 
    ...
    return df_feature_importance,            [roc_auc_train, roc_auc_test,
            ...
            recall_train[0], recall_test[0], recall_train[1], recall_test[1], 0], test_pred_proba.reshape(-1,1),prediction.reshape(-1,1)
...
def lgbm_evaluate(**params):
    ...
    clf = LGBMClassifier(**params, n_estimators = 10000, nthread = 16)
...
if __name__ == "__main__":
    df = pd.read_hdf('./data/df_1516.h5')
    params = {'colsample_bytree': (0.3, 1),
          'learning_rate': (.01, .1), 
          'num_leaves': (20, 50), 
          'subsample': (0.3, 1), 
          'max_depth': (2, 9), 
    ...
    bo.maximize(init_points = 5, n_iter =30,acq='rnd')
    ...
    feature_importance, scor, lgm_oof_train,lgm_oof_test = cv_scores(df,5 , best_params, test_prediction_file_name = 'lightgbm_tune.csv')
    
    # for stack
    np.save('lgm_oof_train',lgm_oof_train)
    np.save('lgm_oof_test',lgm_oof_test)
